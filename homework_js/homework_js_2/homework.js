...
const fullNamePerson = (firstName = "Где", lastName = "имя??") => {
...
const makePerson = (person, canDriveCb, getFullNameCb) => {
    return {
        fullName: () => getFullNameCb(person.firstName, person.lastName),
        age: person.age,
        gender: person.gender,
        canDrive: () => canDriveCb(person.age, person.gender)
    }
};

const personAlex = makePerson(person, canDrive, fullNamePerson);
console.log("task 3: ");
...
const countup = (initValue, finishValue) => {
    if (initValue <= finishValue) {
        console.log(initValue);
        initValue++;
        countup(initValue, finishValue);
    }
    return false;
}
console.log("task 4: ");
